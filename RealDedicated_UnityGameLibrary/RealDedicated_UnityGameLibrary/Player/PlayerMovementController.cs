...
namespace RealDedicated_UnityGameLibrary.Player
{
    class PlayerMovementController : MonoBehaviour
    {
        ...
        void HandleCursorAlignment (Vector3 cursorWorldPosition )
        {
            ...
            float compensatedScale = (float)(0.1 * Vector3.Dot(cursorWorldPosition - mainCameraTransform.position, mainCameraTransform.forward));

            // Make the cursor smaller when close to character
            float cursorScaleMultiplier = Mathf.Lerp(0.7f, 1.0f, (Mathf.InverseLerp(0.5f, 4.0f, (float)motor.facingDirection.magnitude)));
